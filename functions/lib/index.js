...
exports.manualAutoCloseCompletedTickets = exports.sendQuoteSubmissionEmail = exports.sendQuoteRequestEmail = void 0;
    ...
    try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <p><strong>Requested By:</strong> ${(requester === null || requester === void 0 ? void 0 : requester.name) || 'Unknown'}</p>
          </div>
      ...
      `;
        // Send email
        const mailOptions = {
            from: ((_c = functions.config().email) === null || _c === void 0 ? void 0 : _c.user) || process.env.EMAIL_USER,
            to: supplier.email,
...
exports.sendQuoteSubmissionEmail = functions.firestore
    .document('quotes/{quoteId}')
    ...
    try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f0fdf4; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <p><strong>Amount:</strong> $${((_c = newData.amount) === null || _c === void 0 ? void 0 : _c.toLocaleString()) || 'N/A'}</p>
            <p><strong>Valid Until:</strong> ${((_d = newData.validUntil) === null || _d === void 0 ? void 0 : _d.toDate().toLocaleDateString()) || 'N/A'}</p>
      ...
      `;
        ...
        const mailOptions = {
            from: ((_e = functions.config().email) === null || _e === void 0 ? void 0 : _e.user) || process.env.EMAIL_USER,
            to: supplier.email,
...
// NOTE: Commented out temporarily due to billing requirement for Cloud Scheduler
...
//     }
//   })
// Manual trigger for auto-close (for testing without Cloud Scheduler)
exports.manualAutoCloseCompletedTickets = functions.https.onCall(async (data, context) => {
    var _a;
    // Check if user is authenticated and has admin privileges
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
    }
    console.log('🔄 Manual auto-close triggered by:', context.auth.uid);
    try {
        const now = new Date();
        const sevenDaysAgo = new Date(now.getTime() - (7 * 24 * 60 * 60 * 1000));
        console.log('🔍 Looking for Complete tickets older than:', sevenDaysAgo.toISOString());
        ...
        for (const doc of snapshot.docs) {
            ...
            // Find when the ticket was completed from activity log
            const activityLog = ticket.activityLog || [];
            const completedActivity = activityLog.find((log) => {
                var _a, _b;
                return (log.action === 'Status Updated' && ((_a = log.description) === null || _a === void 0 ? void 0 : _a.includes('Complete'))) ||
                    ((_b = log.action) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes('completed'));
            });
            if (!completedActivity) {
                console.log(`⚠️  Ticket ${doc.id}: No completion activity found, skipping`);
                continue;
            }
            const completedDate = ((_a = completedActivity.timestamp) === null || _a === void 0 ? void 0 : _a.toDate) ? completedActivity.timestamp.toDate() : new Date(completedActivity.timestamp);
            const daysSinceCompleted = Math.floor((now.getTime() - completedDate.getTime()) / (24 * 60 * 60 * 1000));
            console.log(`🔍 Ticket ${doc.id}: Completed ${daysSinceCompleted} days ago`);
            if (daysSinceCompleted >= 7) {
                console.log(`✅ Auto-closing ticket ${doc.id} (completed ${daysSinceCompleted} days ago)`);
        ...
        if (closedCount > 0) {
            await batch.commit();
            console.log(`✅ Manual auto-close completed: ${closedCount} tickets closed out of ${processedCount} Complete tickets`);
        }
        else {
            console.log(`ℹ️  Manual auto-close completed: No tickets needed closing out of ${processedCount} Complete tickets`);
        }
        return {
            success: true,
            processedCount,
        ...
        throw new functions.https.HttpsError('internal', 'Auto-close operation failed');
    }
});
