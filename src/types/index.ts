...
// ===== BUDGET YEAR RANGE WORKFLOW =====

export enum BudgetSetupStep {
  YEAR_RANGE_SELECTION = "year_range_selection",
  CATEGORY_CONFIGURATION = "category_configuration", 
  BUDGET_ALLOCATION = "budget_allocation",
  REVIEW_AND_APPROVAL = "review_and_approval",
  SETUP_COMPLETION = "setup_completion"
}

export enum BudgetSetupStatus {
  DRAFT = "draft",
...
export interface BudgetYearRange {
  startYear: number;
  endYear: number;
  years: number[];
}

export interface BudgetSetupWizard {
  id: string;
  buildingId: string;
  currentStep: BudgetSetupStep;
  status: BudgetSetupStatus;
  
  // Step 1: Year Range Selection
  yearRange: BudgetYearRange;
  
  // Step 2: Category Configuration
  categoryTemplate: BudgetCategoryTemplate[];
  
  // Step 3: Budget Allocation
  yearlyAllocations: Record<number, BudgetYearAllocation>;
  ...
  completedAt?: Date;
  completedBy?: string;
  budgetsCreated: string[]; // Budget IDs created from this wizard
...
export interface BudgetCategoryTemplate {
  ...
  approvalThreshold?: number;
  frequency?: PaymentFrequency;
  applyToAllYears: boolean;
  yearSpecificAmounts?: Record<number, number>;
}

export interface BudgetYearAllocation {
  year: number;
  totalBudgetAmount: number;
  categories: BudgetCategoryAllocation[];
  notes?: string;
  isLocked: boolean;
  basedOnPreviousYear?: number; // Year used as template
  adjustmentPercentage?: number; // % adjustment from template
}
...
export interface BudgetSetupProgress {
  wizardId: string;
  currentStep: BudgetSetupStep;
  stepsCompleted: BudgetSetupStep[];
  totalSteps: number;
...
export interface BudgetYearRangeValidation {
  isValid: boolean;
  errors: string[];
  warnings: string[];
...
export interface BudgetReviewSummary {
  wizardId: string;
  yearRange: BudgetYearRange;
  yearSummaries: {
    year: number;
  ...
  totalBudgetAcrossYears: number;
  totalIncomeAcrossYears: number;
  totalExpenditureAcrossYears: number;
  averageYearlyBudget: number;
  netPositionAcrossYears: number;
...
export interface BudgetCompletionSummary {
  wizardId: string;
  buildingId: string;
  yearRange: BudgetYearRange;
  createdBudgets: {
...
export enum FlatLedgerTransactionType {
  SERVICE_CHARGE_DEMAND = "service_charge_demand",
  GROUND_RENT_DEMAND = "ground_rent_demand", 
  PAYMENT = "payment",
...
export enum FlatLedgerTransactionStatus {
  PENDING = "pending",
  PROCESSED = "processed",
  CANCELLED = "cancelled",
  REVERSED = "reversed"
...
export interface FlatLedgerTransaction {
  id: string;
  flatId: string;
  buildingId: string;
  flatNumber: string;
  ...
  // Transaction Details
  type: FlatLedgerTransactionType;
  status: FlatLedgerTransactionStatus;
  description: string;
  reference: string; // Demand ID, Payment ID, etc.
  
  // Financial Data
  debitAmount: number; // Money owed by resident
  creditAmount: number; // Money paid by resident or credits applied
  runningBalance: number; // Balance after this transaction
  
  // Dates
  transactionDate: Date;
  dueDate?: Date; // For demands
  processedDate?: Date;
  
  // Additional Context
  quarter?: string; // e.g., "Q1 2024"
  period?: string; // e.g., "Jan-Mar 2024"
  category?: string; // Service charge, ground rent, etc.
  notes?: string;
...
export interface FlatLedgerSummary {
  ...
  // Period Analysis
  currentQuarterBalance: number;
  previousQuarterBalance: number;
...
export interface FlatLedgerFilter {
  dateFrom?: Date;
  dateTo?: Date;
  transactionTypes?: FlatLedgerTransactionType[];
  status?: FlatLedgerTransactionStatus[];
  quarters?: string[];
  minAmount?: number;
  maxAmount?: number;
...
export interface FlatLedgerReport {
  flatId: string;
  buildingId: string;
  flatNumber: string;
  residentName: string;
  ...
  transactions: FlatLedgerTransaction[];
  ...
  insights: {
    ...
    largestTransaction: FlatLedgerTransaction;
    recommendations: string[];
  };
